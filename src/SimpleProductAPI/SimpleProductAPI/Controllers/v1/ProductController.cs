...
{
    /// <summary>
    /// API controller for v1 product operations.
    ...
    public class ProductController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<Product>>> GetProductsAsync()
        {
            ...
            _logger.LogInformation("v1 GetProductsAsync returned {Count} products", products?.Count ?? 0);
            return Ok(products);
        }
        ...
        public async Task<ActionResult<Product>> GetProductAsync(int id)
        {
            ...
            if (product == null)
            {
                _logger.LogWarning("v1 GetProductAsync: not found id={Id}", id);
                return NotFound();
            }
            _logger.LogInformation("v1 GetProductAsync: found id={Id}", id);
            return Ok(product);
        }
        ...
        public async Task<ActionResult> UpdateProductDescriptionAsync(int id, [FromBody] UpdateDescriptionDto dto)
        {
            ...
            {
                _logger.LogWarning("v1 UpdateProductDescriptionAsync: invalid payload for id={Id}", id);
                return BadRequest("v1 Description is required");
            }

            var result = await _productService.UpdateProductDescriptionAsync(id, dto.Description);
            if (!result)
            {
                _logger.LogWarning("v1 UpdateProductDescriptionAsync: product not found for id={Id}", id);
                return NotFound();
            }

            _logger.LogInformation("v1 UpdateProductDescriptionAsync: description updated for id={Id}", id);
            return NoContent();
        }
    }
}
